...
export const copy = {
  ...
  billing: {
    ...
    findOrCreate: "Chercher ou créer un client…",
    findClient: "Chercher un client…",
    namePlaceholder: "Ex. Boulangerie Saint-Roch",
    nameTaken: "Ce client existe déjà.",
    openExisting: (name: string) => `Ouvrir ${name}`,
    noMatch: "Aucun client ne correspond.",
    cancel: "Annuler",
    create: "Créer le client",
    close: "Fermer",
    saved: "Enregistré",
    rateHint: "Utilisé pour toute ligne avec des heures et sans taux à elle. Le changer recalcule ces lignes.",
    deleteClientShort: "Supprimer le client",
    emptyTitle: (name: string) => `Rien encore pour ${name}`,
    emptyBody: "Ajoute une première ligne, ou copie des lignes de ton Excel et colle-les n'importe où sur cette page (Ctrl+V).",
    rateLine: (rate: string) => `Taux : ${rate} $/h — modifier`,
    createNamed: (name: string) => `Créer le client « ${name} »`,
    active: "Actifs",
    archived: "Archivés",
    ...
    emptyRows: "Aucune ligne dans ce filtre.",
    firstRow: "Aucune ligne encore. « Ajouter une ligne », ou copie des lignes de ton Excel et colle-les ici (Ctrl+V).",
    imported: (n: number) => `${n} ligne${n > 1 ? "s" : ""} importée${n > 1 ? "s" : ""}`,
    titlePlaceholder: "Nouvelle ligne…",
    ...
    productsIntro:
      "Les services que vous vendez souvent. Ajoutés à une feuille client en un clic, avec leur prix du jour — changer un prix ici ne touche pas les lignes déjà écrites.",
    productName: "Produit",
    productPrice: "Prix ($)",
    productPlaceholder: "Ex. Forfait Vitrine",
    productDetailPlaceholder: "Ce que ça inclut",
    addProduct: "Ajouter un produit",
    noProducts: "Aucun produit. Ajoute ceux que tu vends souvent.",
    ...
    taxesNote: "Les taxes ne comptent pas comme revenu : les totaux ci-dessus sont hors taxes.",
    showOwing: {
      pending: "Voir à qui facturer",
      invoiced: "Voir qui relancer",
      outstanding: "Voir qui doit de l'argent",
    },
    overdue: "En souffrance",
    taxesCollected: "Taxes",
    taxesHint: "TPS + TVQ encaissées",
    beforeTax: "Hors taxes",
    ...
    copyShort: "Copier",
    markAllInvoicedShort: "Tout facturer",
    markAllPaidShort: "Tout payé",
    invoiceHeading: "à facturer",
    totalLabel: "Total",
    copied: (n: number) => `${n} ligne${n > 1 ? "s" : ""} copiée${n > 1 ? "s" : ""} — colle dans QuickBooks`,
    copyFailed: "Pas pu copier. Réessaie.",
    bulkMoved: (n: number, status: string, sum: string) =>
      `${n} ligne${n > 1 ? "s" : ""} → ${status} · ${sum}`,
    clientStatus: { active: "Actif", archived: "Archivé" },
    statusChanged: (name: string, status: string) => `${name} : ${status}`,
    statusHint: "Un client archivé quitte la liste des actifs et ses montants sortent des totaux. Rien n'est supprimé.",
    deleteClient: "Supprimer le client (aucune ligne)",
    clientDeleted: (name: string) => `${name} supprimé`,
    rate: "Taux par défaut",
    settings: "Modifier le client",
    rateSuffix: "$/h",
  ...
  task: {
    ...
    stepDelete: "Supprimer l'étape",
    stepCount: (done: number, total: number) => `${done}/${total}`,
    ...
    repeated: "Prochaine occurrence créée",
    repeatHint: "La prochaine sera créée quand tu termineras celle-ci.",
    nobody: "Personne",
    label: "Étiquette",
    important: "Important",
    status: "Statut",
