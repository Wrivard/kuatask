...
- **« Nous deux » is a flag, not a list of assignees** (0024). Asked for directly. With two people, "both" is one more value rather than an array: `shared boolean` beside `assignee_id`, exclusive by a CHECK constraint. Everything reading `assignee_id` keeps working — a shared task has none — and only the surfaces that should know learn to: both faces on the row and the card, a « Nous deux » chip in the modal, a column of its own on the board (drag a card into it), `@nous` in the composer and the palette, and the A key cycling nobody → each person → both. Both personal lenses include shared tasks, and both progress rings count them; a shared task that showed in neither would be the one nobody picks up. An array would have meant rewriting all 33 files that read the column, for a workspace that holds two people by design.
- **Subtasks are a checklist, not tasks** (0026). A task carries a bucket, a person, a status, a colour, a position, and it counts towards the streak and the day's progress; a step inside one carries none of that. Making steps tasks would put every « imprimer le devis » in the list, on the board, in the calendar and in the counts — the opposite of what a subtask is for. So `subtasks` is a title, a checkbox and an order, scoped through its task (no workspace column: the task already holds one, and two columns that can disagree is one more thing to keep honest). They live in the store because the list shows « 2/5 » on the row, and a count that arrives after the row has drawn is a row that moves under your eyes.
- **Recurrence is a rule on the task, applied at completion** (0026). `tasks.recur` holds the rule; ticking a recurring task writes the next occurrence immediately, client-side, with its steps copied unticked. No scheduler: nothing would be running while nobody is — the free plan's database sleeps — and a cron would keep making occurrences of a routine that has been abandoned. The next date counts from the day it was **due**, not from today, so a weekly task finished three days late does not drift three days later every week. With no date at all it counts from the day it was done, the only honest reading of a rule with no anchor.
